...
<html>
<head>
    <title>Property Type List <?= date("d-m-Y") ?> </title>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">Facilities List
                            <div  class="site_nav" >
                            ...
                            <table id="dt_tableTools" class=" table table-striped table-bordered ">
                                ...
                                <?php foreach($records as $k => $row): ?>
                                    <tr>
                                        <td><?= $k+1 ?></td>
                                        <td> <?= $row->code ?> </td>
                                        <td> <i class="<?= $row->image ?>" ></i> </td>
                                        <td>  <?= $row->title ?> </td>
                                        <td><?= $row->create_date ?></td>
                                        <td><?= $row->user ?></td>
                                        <td> <a class=" btn btn-warning   fa fa-pencil-square-o" href="<?= current_url() ."/manage/?id=$row->id" ?>"  ></a> <a class=" btn btn-danger fa fa-times " onclick="row.remove($(this))" data-id="<?= $row->id ?>" > </a> </td>
                                    </tr>
                                <?php endforeach; ?>
                                </tbody>
                            </table>
    ...
    row  = {
        remove: function(self){
            if(confirm("Are You Sure Do your can to delete this record ???  ")){
                $.ajax({
                    url : "<?= base_url() ?>admin/property_type/remove",
